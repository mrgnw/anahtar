...
export function sqliteAdapter(db: Database.Database, options: SqliteAdapterOptions = {}): AuthDB {
	...
	return {
		...
		getSession(tokenHash: string): (SessionRecord & { email: string }) | null {
			const row = db
				...
				.get(tokenHash) as { id: string; user_id: string; expires_at: number; email: string } | undefined;
			if (!row) return null;
			return {
				id: row.id,
				userId: row.user_id,
		...
		storeChallenge(challenge: string, userId: string, expiresAt: number) {
			...
			db.prepare(`INSERT INTO ${t.challenges} (challenge, user_id, expires_at) VALUES (?, ?, ?)`).run(
				challenge,
				userId,
				expiresAt
			);
